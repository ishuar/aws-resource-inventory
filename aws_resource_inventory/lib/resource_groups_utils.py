...
def get_all_tagged_resources_across_services(
    ...
    tagging_client = get_scan_client(session, "resourcegroupstaggingapi", region)
...
def scan_all_tagged_resources(
    ...
    # Failures propagate (ADR-0010): this path has no per-service
    # granularity, so either half failing fails the whole region — the
    # caller records it as a region-level ScanError. Swallowing here is
    # how a denied Tagging API used to read as "no tagged resources".
    with ThreadPoolExecutor(max_workers=2) as executor:
        # Run Resource Groups API and Auto Scaling scans in parallel
