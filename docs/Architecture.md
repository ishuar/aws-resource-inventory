...
| `aws_resource_inventory/lib/paths.py` | Where the tool puts files on disk | One resolver: XDG variable first, one documented fallback, the same on every platform (ADR-0009) |
